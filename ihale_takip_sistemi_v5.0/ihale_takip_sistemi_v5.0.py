...
# --- KÜTÜPHANE SEÇİMİ (PySide6 veya PyQt6) ---
try:
    ...
    from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                                 ...
                                 QTabWidget, QScrollArea, QComboBox, QListView, QFileDialog, 
    ...
    from PySide6.QtGui import (QIntValidator, QRegularExpressionValidator, QFont, 
                              QColor, QPixmap, QImage, QIcon, QTextCharFormat, QBrush, QPainter)
except ImportError:
    ...
    from PyQt6.QtGui import (QIntValidator, QRegularExpressionValidator, QFont, 
                              QColor, QPixmap, QImage, QIcon, QTextCharFormat, QBrush, QPainter)
...
class NewTenderDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Parti Miktarı
        layout.addWidget(QLabel("<b>Parti Miktarı (Örn: 50 Adet, 100 kg):</b>"))
        self.fields["Parti Miktarı"] = QLineEdit()
        self.fields["Parti Miktarı"].setPlaceholderText("Parti miktarını giriniz")
        ...
        # Turkish Currency Parsing: Remove dots, replace comma with dot
            ...
            for i in range(1, parti_sayisi + 1):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 0, 0, 0, '', ?)
                """, (ikn, firma, ihale, i, teslim_tarihi, parti_miktari, parti_tutari, audit_info))
...
class NewBatchDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.miktar_edit.setPlaceholderText("Örn: 50 Adet, 100 kg")
        ...
        self.amount_edit.textChanged.connect(self._format_currency)
    ...
    def _format_currency(self, text):
        """Binlik nokta ayracı ile para birimi formatlar."""
        line_edit = self.sender()
        if not text:
            return
        cursor_pos = line_edit.cursorPosition()
        text_before = text[:cursor_pos]
        digit_count = len(text_before.replace('.', ''))
        line_edit.blockSignals(True)
        clean = text.replace('.', '')
        if ',' in clean:
            parts = clean.split(',', 1)
            int_part, dec_part = parts[0], parts[1][:2]
            has_comma = True
        else:
            int_part, dec_part, has_comma = clean, "", False
        if int_part.isdigit():
            formatted = "{:,}".format(int(int_part)).replace(',', '.')
            new_text = formatted + ("," + dec_part if has_comma else "")
        else:
            new_text = clean
        line_edit.setText(new_text)
        new_cursor = 0
...
class DetailWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        btn_new_batch.setObjectName("PrimaryBtn")
        btn_new_batch.setStyleSheet("""
            QPushButton {
        ...
        self.table = QTableWidget(); self.table.setColumnCount(15)
        self.table.setAlternatingRowColors(True)
        self.table.setHorizontalHeaderLabels(["IKN", "Firma", "İhale Adı", "Parti", "Tarih", "Miktar", "Tutar", "Ambar", "Test B.", "Test S.", "Rapor", "Kabul", "Ödeme", "Açıklama", "İşlem"])
        ...
        self.table.setColumnWidth(13, 200) # Aciklama
        self.table.setColumnWidth(14, 150) # Islem
    ...
    def export_excel(self):
        try:
            ...
            QMessageBox.warning(self, "Hata", "Excel'e dışa aktarma işlemi için 'pandas' kütüphanesi gereklidir.")
            return
            
        file_path, _ = QFileDialog.getSaveFileName(self, "Excel Olarak Kaydet", "ihale_veritabanı_aktarim.xlsx", "Excel Dosyaları (*.xlsx)")
        if not file_path:
            return
            
        try:
            all_data = fetch_data()
            cols = ["IKN", "Firma", "İhale Adı", "Parti", "Son Teslim Tarihi", "Parti Miktarı", "Parti Tutarı", 
                    "Ambar Teslimi", "Testler Başladı", "Test Sonuçları Geldi", "Kabul Raporu İmzada", 
                    "Kabul Yapıldı", "Fatura / Dosya Kapandı", "Ödeme Emri", "Açıklama", "Son Güncelleme"]
            
            rows = []
            for r in all_data:
                miktar = str(r[16]) if len(r) > 16 and r[16] else ""
                row = [
                    r[1], r[2], r[3], r[4], 
                    format_date_tr(str(r[5])) if r[5] else "", 
                    miktar, 
                    float(r[6]) if r[6] else 0.0,
                    "Evet" if len(r)>7 and r[7]==1.0 else "Hayır",
                    "Evet" if len(r)>8 and r[8]==1.0 else "Hayır",
                    "Evet" if len(r)>9 and r[9]==1.0 else "Hayır",
                    "Evet" if len(r)>10 and r[10]==1.0 else "Hayır",
                    "Evet" if len(r)>11 and r[11]==1.0 else "Hayır",
                    "Evet" if len(r)>12 and r[12]==1.0 else "Hayır",
                    "Evet" if len(r)>13 and r[13]==1.0 else "Hayır",
            ...
            df_active = df_all[df_all["Kabul Yapıldı"] == "Hayır"]
            
            firm_summary = get_firm_summary_data("Tümü")
            df_firms = pd.DataFrame([
                {"Firma": f["name"], "İhale Sayısı": f["tender_count"], "Toplam Parti": f["part_count"], "Toplam Hacim (TL)": float(f["volume"])}
                for f in firm_summary
            ])
            ...
            if CURRENT_USER_ROLE == "admin":
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("SELECT id, timestamp, user, action, details FROM logs ORDER BY id DESC")
                logs = cursor.fetchall()
                conn.close()
                df_logs = pd.DataFrame(logs, columns=["ID", "Tarih", "Kullanıcı", "İşlem", "Detaylar"])

            with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                df_all.to_excel(writer, sheet_name="Tüm Kayıtlar", index=False)
                df_active.to_excel(writer, sheet_name="Devam Eden İşler", index=False)
                df_firms.to_excel(writer, sheet_name="Firma Özetleri", index=False)
                if df_logs is not None:
                    df_logs.to_excel(writer, sheet_name="Sistem Kayıtları", index=False)
                    
            QMessageBox.information(self, "Başarılı", f"Tüm veriler başarıyla {file_path} dosyasına aktarıldı.")
            
        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Dışa aktarma işlemi başarısız: {e}")
    ...
    def apply_filters(self):
        ...
        for row_idx, r in enumerate(filtered):
            ...
            m_item = QTableWidgetItem(miktar_val)
            m_item.setTextAlignment(Qt.AlignCenter)
            self.table.setItem(row_idx, 5, m_item)
            ...
            for c, idx in enumerate([7, 8, 9, 10, 11, 13], 7): # Skip 12 (Fatura)
                val = r[idx] if idx < len(r) else 0.0
                it = QTableWidgetItem("✓" if val==1.0 else "○"); it.setTextAlignment(Qt.AlignCenter)
                self.table.setItem(row_idx, c, it)
            self.table.setItem(row_idx, 13, QTableWidgetItem(str(r[14]) if len(r)>14 else ""))
            ...
            self.table.setCellWidget(row_idx, 14, action_widget)
...
class AboutDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Info Text
        info_text = """
        <h3 style='color: #6366f1; margin-bottom:0;'>İhale Takip Uygulaması v5.00</h3>
        <br>
