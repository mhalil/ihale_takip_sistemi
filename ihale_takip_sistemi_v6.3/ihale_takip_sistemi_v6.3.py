...
class NewBatchDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.miktar_edit.setPlaceholderText("Örn: 1.000,00")
...
class EditDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.sozlesme_tarih_edit = QDateEdit()
        self.sozlesme_tarih_edit.setCalendarPopup(True)
        self.sozlesme_tarih_edit.setEnabled(False)
        try:
            # Sozlesme Tarihi index is likely 17 based on Parti Miktari being 16
            if len(self.record) > 17 and self.record[17]:
                d = datetime.strptime(str(self.record[17])[:10], "%Y-%m-%d")
                self.sozlesme_tarih_edit.setDate(QDate(d.year, d.month, d.day))
            else:
                self.sozlesme_tarih_edit.setDate(QDate.currentDate())
        except:
            self.sozlesme_tarih_edit.setDate(QDate.currentDate())
        layout.addWidget(QLabel("Sözleşme Tarihi:"))
        layout.addWidget(self.sozlesme_tarih_edit)
...
class BulkEditDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        layout.addWidget(QLabel("<i>( Açık Gri Kare : Mevcut Durumu Değiştirme _____ ✓ : Onayla _____ Boş Kare : Onayı İptal Et / Onaylama )</i>"))
...
class DetailWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table = QTableWidget(); self.table.setColumnCount(16)
        self.table.setAlternatingRowColors(True)
        self.table.setHorizontalHeaderLabels(["IKN", "Firma", "İhale Adı", "Sözl. Tarihi", "Parti", "Parti Tarihi", "Miktar", "Tutar", "Ambar", "Test B.", "Test S.", "Rapor", "Kabul", "Ödeme", "Açıklama", "İşlem"])
        ...
        self.table.setColumnWidth(7, 120) # Tutar
        
        # Durum Sütunları (Ambar, Test B., Test S., Rapor, Kabul, Ödeme)
        for i in range(8, 14):
            self.table.setColumnWidth(i, 80)
            
        self.table.setColumnWidth(14, 200) # Aciklama
        self.table.setColumnWidth(15, 150) # Islem
    ...
    def apply_filters(self):
        ...
        for row_idx, r in enumerate(filtered):
            ...
            self.table.setItem(row_idx, 7, t_item)
            
            for c, idx in enumerate([7, 8, 9, 10, 11, 13], 8): # Table indices 8-13
            ...
            self.table.setItem(row_idx, 14, QTableWidgetItem(str(r[14]) if len(r)>14 else ""))
            ...
            self.table.setCellWidget(row_idx, 15, action_widget)
...
class AboutDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Info Text
        info_text = """
        <h3 style='color: #6366f1; margin-bottom:0;'>İhale Takip Uygulaması v6.3</h3>
        <br>
